	...
	switch ($_ACT)
	{
		...
		case 'program_selection':
			...
			else
			{
				...
				if ($policy)
				{
					$policy->policy_data->search_programs_clinics_selected();

					header('Result: 1');

					$_TPL = $smarty->createTemplate(TPL.'dms/query_o_create_program_submit_form.tpl');
					$_TPL->assign(
					[
						'clinics'			=> &$policy->policy_data->programs_adult['clinics'],
		...
		default:
			...
			if ($policy)
			{
				$policy->policy_data->search_programs_adult();

				header('Result: 1');

				$_TPL = $smarty->createTemplate(TPL.'dms/query_o_create_program.tpl');
				$_TPL->assign(
				[
