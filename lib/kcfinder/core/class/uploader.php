...
class uploader {
	...
	/**
	 * Helper property.
	 * Local filesystem path to the Type Directory
	 ...
	 * Linked to first $_FILES element
	 * 
	 * @var array
	 */
	protected $file;
	...
	/**
	 * Session array.
	 * Please use this property instead of $_SESSION
	 * 
	 * @var array
	 */
	protected $session;
	...
	public function __construct() {
		...
		if ((! isset ( $driver ) || ($driver === false)) && (image::getDriver ( array (
				$this->imageDriver 
		) ) === false))
		...
		if (! $this->config ['disabled']) {
			...
			if (! is_dir ( $this->typeDir )) {
				if (! mkdir ( $this->typeDir, $this->config ['dirPerms'] ))
					$this->backMsg ( "Cannot create {dir} folder.", array (
							'dir' => $this->type 
					) );
	...
	protected function checkUploadedFile(array $aFile = null) {
		...
		if ($file ['error'])
			return ($file ['error'] == UPLOAD_ERR_INI_SIZE) ? $this->label ( "The uploaded file exceeds {size} bytes.", array (
