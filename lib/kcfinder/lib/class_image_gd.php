...
class image_gd extends image {
	...
	public function resize($width, $height) {
		...
		if (! $height)
			$height = 1;
		return ((false !== ($img = new image_gd ( array (
				$width,
				$height 
		) ))) && $img->imageCopyResampled ( $this ) && (false !== ($this->image = $img->image)) && (false !== ($this->width = $img->width)) && (false !== ($this->height = $img->height)));
	}
	...
	public function watermark($file, $left = false, $top = false) {
		...
		$w = imagesx ( $wm );
		$h = imagesy ( $wm );
		$x = ($left === true) ? 0 : (($left === null) ? round ( ($this->width - $w) / 2 ) : ((($left === false) || ! preg_match ( '/^\d+$/', $left )) ? ($this->width - $w) : $left));
		$y = ($top === true) ? 0 : (($top === null) ? round ( ($this->height - $h) / 2 ) : ((($top === false) || ! preg_match ( '/^\d+$/', $top )) ? ($this->height - $h) : $top));
	...
	protected function getImage($image, &$width, &$height) {
		if (is_resource ( $image ) && (get_resource_type ( $image ) == "gd")) {
			...
			$image = ($t == IMAGETYPE_GIF) ? @imagecreatefromgif ( $image ) : (($t == IMAGETYPE_WBMP) ? @imagecreatefromwbmp ( $image ) : (($t == IMAGETYPE_JPEG) ? @imagecreatefromjpeg ( $image ) : (($t == IMAGETYPE_PNG) ? @imagecreatefrompng ( $image ) : (($t == IMAGETYPE_XBM) ? @imagecreatefromxbm ( $image ) : false))));
	...
	protected function gdColor() {
		...
		if (count ( $args [0] ) == 3) {
			...
			$g = hexdec ( "$g$g" );
			$b = hexdec ( "$b$b" );
		} elseif ((count ( $args ) == 3) && preg_match ( $exprByte, $args [0] ) && preg_match ( $exprByte, $args [1] ) && preg_match ( $exprByte, $args [2] )) {
			list ( $r, $g, $b ) = $args;
		} else
			return false;
	...
	protected function imageCopyResampled($src, $dstX = 0, $dstY = 0, $srcX = 0, $srcY = 0, $dstW = null, $dstH = null, $srcW = null, $srcH = null) {
