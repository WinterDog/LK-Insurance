...
class lessc {
	...
	public static $TRUE = array (
			"keyword",
			"true" 
	);
	public static $FALSE = array (
			"keyword",
			"false" 
	);
	...
	protected function compileBlock($block) {
		switch ($block->type) {
			...
			case "directive" :
				...
				$this->compileNestedBlock ( $block, array (
						$name 
				) );
				break;
			default :
	...
	protected function compileMedia($media) {
		...
		$this->scope = $this->makeOutputBlock ( $media->type, array (
				$query 
		) );
	 ...
	 * Comments are not deduplicated. If a
...
class lessc_parser {
	...
	protected static $supressDivisionProps = array (
			'/border-radius$/i',
			'/^font$/i' 
	);
	...
	protected $lineDirectives = array (
			"charset" 
	);
	...
	protected function parseChunk() {
		...
		if ($this->tag ( $tag, true ) && $this->argumentDef ( $args, $isVararg ) && ($this->guards ( $guards ) || true) && $this->literal ( '{' )) {
			$block = $this->pushBlock ( $this->fixTags ( array (
					$tag 
			) ) );
			$block->args = $args;
